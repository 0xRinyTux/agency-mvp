        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16">
          ...
          <div>
            <h2 className="text-3xl md:text-4xl font-extrabold mb-8 tracking-tight text-slate-900">
              {data.contact?.title || "Dove Trovarci"}
            </h2>
            ...
            <div className="space-y-8 mb-10">
              <div className="flex items-start">
                <div 
                  ...
                  style={{ backgroundColor: data.meta?.primary_color || '#e11d48' }}
                >
                ...
                <div>
                  <h3 className="text-xl font-bold mb-1 text-slate-900">
                    {data.contact?.phone_label || "Chiamaci"}
                  </h3>
                  <p className="text-slate-600 font-medium">
                    {data.contact?.phone_sublabel || "Siamo a tua disposizione"}
                  </p>
                  {data.contact?.phone && (
                    <a href={`tel:${data.contact.phone.replace(/\s+/g, '')}`} className="text-3xl font-extrabold mt-2 block hover:opacity-80 transition-opacity" style={{ color: data.meta?.primary_color || '#e11d48' }}>
                      {data.contact.phone}
                    </a>
                  )}
                </div>
              ...
              <div className="flex items-start">
                ...
                <div>
                  <h3 className="text-xl font-bold mb-1 text-slate-900">
                    {data.contact?.address_label || "Indirizzo"}
                  </h3>
                  <p className="text-slate-600 font-medium text-lg">
                    {data.contact?.address || "Contattaci per la posizione"}
                  </p>
                </div>
              </div>
            </div>
            ...
            <div className="bg-slate-50 rounded-[1.5rem] p-8 border border-slate-200">
              <h3 className="text-xl font-bold mb-6 flex items-center text-slate-900">
                ...
                {data.contact?.hours_title || "Orari di Apertura"}
              </h3>
              <ul className="space-y-4">
                {data.contact?.opening_hours && Object.entries(data.contact.opening_hours).map(([day, hours]) => (
