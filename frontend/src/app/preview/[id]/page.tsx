...
export default async function PreviewPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const schemaData = {
    ...
    "openingHours": mockData?.contact?.opening_hours ? Object.entries(mockData.contact.opening_hours).map(([day, hours]) => `${day} ${hours}`) : []
  };
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900 font-sans pb-24 md:pb-0">
      ...
      <style dangerouslySetInnerHTML={{__html: `
        ::selection {
          background-color: ${mockData?.meta?.primary_color || '#e11d48'}40;
        }
      `}} />
        ...
        <div className="max-w-7xl mx-auto px-6 h-20 flex items-center justify-between">
          <div className="text-xl md:text-2xl font-extrabold text-slate-900 flex items-center">
            <div 
              ...
              style={{ backgroundColor: mockData?.meta?.primary_color || '#e11d48' }}
            >
              <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 10V3L4 14h7v7l9-11h-7z" />
              </svg>
            </div>
            <span className="truncate">{mockData?.meta?.company_name || "Anteprima Sito"}</span>
          </div>
